"""
weak - Discord Bot
Bleed-inspired multipurpose bot with full Lavalink v4 music via Wavelink 3.x.
...
class WeakBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        """Connect Wavelink nodes BEFORE bot is ready"""
        ...
        node = wavelink.Node(
            ...
            retries=None  # Retry forever
        )
        
        try:
            await wavelink.Pool.connect(nodes=[node], client=self)
            logger.info("Wavelink node connected successfully")
        except Exception as e:
            logger.error(f"Failed to connect Wavelink node: {e}")
            raise
    
    async def on_ready(self):
        logger.info(f"weak logged in as {self.user} (ID: {self.user.id}) | {len(self.guilds)} guilds")
        await self.change_presence(
            activity=discord.Activity(
                type=discord.ActivityType.watching, 
                name=f"{len(self.guilds)} servers | {CONFIG.get('prefix', '.')}help"
            )
        )
...
async def on_wavelink_node_ready(payload: wavelink.NodeReadyEventPayload):
    logger.info(f"Wavelink node ready: {payload.node.identifier} | Session: {payload.session_id}")
...
# ─── MUSIC COG (Wavelink 3.x / Lavalink v4) ─────────────────────────────────
...
class Music(commands.Cog):
    ...
    async def get_player(self, ctx: commands.Context) -> Optional[wavelink.Player]:
        ...
        if not ctx.voice_client:
            ...
            try:
                ...
                await ctx.send(embed=error_embed(f"Failed to connect: {e}"))
                return None
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        try:
            ...
            return await ctx.send(embed=error_embed(f"Search failed: {e}"))
    ...
    async def queue(self, ctx: commands.Context):
        ...
        for i, track in enumerate(queue_list[:20], 1):
            minutes = track.length // 60000 if hasattr(track, 'length') and track.length else 0
            seconds = (track.length % 60000) // 1000 if hasattr(track, 'length') and track.length else 0
            desc += f"**{i}.** [{track.title}]({track.uri}) | {minutes}:{seconds:02d}\n"
        
        embed = weak_embed(title="Queue", description=desc)
        embed.add_field(name="Total", value=f"{len(queue_list)} tracks", inline=True)
        if player.current:
